...
const DarkLuxuryLayout = ({ data, theme, guest, onAddRSVP, rsvps, embedded = false }: DarkLuxuryLayoutProps) => {
  ...
  const AccentText = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
    <span className={`text-[var(--theme-primary)] ${className}`}>
      {children}
    </span>
  );
  ...
  const AccentDivider = () => (
    <div className="flex items-center justify-center gap-4 py-12 opacity-80">
      <div className="h-[1px] w-12 bg-[var(--theme-primary)] opacity-50"></div>
  ...
  return (
    ...
    >
      {data?.bgImageUrl && <div className="fixed inset-0 bg-[var(--theme-bg)] opacity-95 pointer-events-none" />}
      

      {data?.musicUrl && <audio ref={audioRef} src={data.musicUrl} loop />}

      {/* === LOCK SCREEN (COVER) === */}
      <div className={`fixed inset-0 z-50 bg-[var(--theme-bg)] transition-transform duration-[1.5s] ease-[cubic-bezier(0.87,0,0.13,1)] flex flex-col items-center justify-center p-6 ${isOpen ? '-translate-y-full' : 'translate-y-0'}`}>
        ...
        <div className="relative z-10 text-center w-full max-w-lg border border-[var(--theme-border)] p-7 md:p-12 bg-[var(--theme-bg)]">
          <p className={`${theme.fontSans} tracking-[0.3em] text-[var(--theme-muted)] text-xs uppercase mb-6`}>THE WEDDING OF</p>

          <h1 className={`${theme.fontSerif} text-5xl md:text-7xl mb-4 leading-tight`}>
            <AccentText>{data?.couple?.groom?.nickname}</AccentText>
            <span className="block text-2xl my-2 text-[var(--theme-muted)] font-thin">&</span>
            <AccentText>{data?.couple?.bride?.nickname}</AccentText>
          </h1>

          <div className="w-[1px] h-16 bg-[var(--theme-primary)] opacity-40 mx-auto my-6"></div>

          {/* Guest Name */}
          <div className="mb-8">
            <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)] italic tracking-wide mb-1`}>Kepada Yth,</p>
          ...
          >
            <div className="relative w-16 h-16 flex items-center justify-center rounded-full border shadow-lg transition-all duration-700 bg-black/50" style={{ borderColor: 'var(--theme-primary)' }}>
              {isUnlocked ? (
            ...
            <span className="text-[10px] tracking-[0.2em] uppercase font-bold text-[var(--theme-muted)] group-hover:text-white transition-colors">{isUnlocked ? 'Membuka...' : 'Buka Undangan'}</span>
          </motion.button>
        </div>
      </div>
      ...
      <div ref={contentRef} className={`relative z-10 transition-opacity duration-1000 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <header className="relative h-screen flex flex-col items-center justify-center text-center px-4 overflow-hidden">
          ...
          <div className="relative z-10">
            <motion.p 
              ...
              className={`${theme.fontSerif} text-4xl md:text-5xl text-[var(--theme-text)] mb-2 tracking-tight leading-tight`}
            >
            ...
            <motion.p 
              ...
              className={`${theme.fontSerif} text-xl md:text-2xl text-[var(--theme-muted)]`}
            >
        ...
        {data?.quoteText && (
          ...
          >
            <section className="py-16 px-6 text-center max-w-3xl mx-auto relative">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1px] h-20 bg-[var(--theme-border)]"></div>
              
              <h3 className={`${theme.fontSerif} text-3xl mb-8`} style={{ color: 'var(--theme-primary)' }}>Kutipan Suci</h3>
              <blockquote className={`${theme.fontSerif} text-lg md:text-3xl italic leading-relaxed text-[var(--theme-text)] border-l-2 pl-6 md:pl-10 text-left md:text-center md:border-l-0 md:border-t-2 md:pt-10 relative`} style={{ borderColor: 'var(--theme-primary)' }}>
              ...
              <p className={`${theme.fontSans} text-xs font-bold mt-10 uppercase tracking-widest text-[var(--theme-muted)]`}>
                — {data?.quoteSource}
              </p>
            </section>
            <AccentDivider />
        ...
        <section className="py-16 px-6 relative overflow-hidden">
          ...
          <motion.h2 
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className={`text-center ${theme.fontSerif} text-4xl md:text-5xl mb-12`}
          >
            <AccentText>Sang Mempelai</AccentText>
          </motion.h2>

          <div className="grid md:grid-cols-2 gap-14 max-w-5xl mx-auto relative z-10">
            {/* Groom */}
            <motion.div 
            ...
            >
              <div className="relative w-56 h-56 mx-auto mb-10">
                {/* Glowing Ring */}
                <div className="absolute inset-0 rounded-full border-[2px] shadow-lg group-hover:rotate-180 transition-all duration-1000 border-dashed" style={{ borderColor: 'var(--theme-primary)' }}></div>
              ...
              <h3 className={`${theme.fontSerif} text-3xl md:text-4xl mb-3 text-[var(--theme-text)]`}>{data?.couple?.groom?.fullName}</h3>
              <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)] uppercase tracking-[0.2em] mb-2`}>Putra dari</p>
              <p className={`${theme.fontSerif} italic text-lg`} style={{ color: 'var(--theme-primary)' }}>{data?.couple?.groom?.fatherName} <span className="text-[var(--theme-muted)]">&</span> {data?.couple?.groom?.motherName}</p>

              {data?.couple?.groom?.instagram && (
                <a href={`https://instagram.com/${data.couple.groom.instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 mt-6 px-6 py-2 border rounded-full text-xs uppercase tracking-widest text-[var(--theme-muted)] hover:text-black transition-all group/ig" style={{ borderColor: 'var(--theme-primary)' }} onMouseOver={(e) => { e.currentTarget.style.backgroundColor = theme.primaryHex; }} onMouseOut={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; }}>
            ...
            >
              ...
              <h3 className={`${theme.fontSerif} text-3xl md:text-4xl mb-3 text-[var(--theme-text)]`}>{data?.couple?.bride?.fullName}</h3>
              <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)] uppercase tracking-[0.2em] mb-2`}>Putri dari</p>
              <p className={`${theme.fontSerif} italic text-lg`} style={{ color: 'var(--theme-primary)' }}>{data?.couple?.bride?.fatherName} <span className="text-[var(--theme-muted)]">&</span> {data?.couple?.bride?.motherName}</p>
        ...
        <section id="events-section" className="py-10 px-6 overflow-hidden">
          ...
          >
            ...
            <p className={`text-center ${theme.fontSans} text-xs text-[var(--theme-muted)] tracking-[0.3em] uppercase mb-16`}>Please join us</p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            {data.events?.akad?.enabled !== false && (
              <motion.div 
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.8 }}
                className="bg-[var(--theme-surface)] border border-[var(--theme-border)] hover:border-[var(--theme-primary)] p-6 md:p-10 text-center relative overflow-hidden group transition-colors duration-500"
              >
                <div className="absolute top-0 left-0 w-full h-[2px] bg-[var(--theme-primary)] opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20"></div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="my-8 w-12 h-[1px] bg-[var(--theme-border)] mx-auto relative">
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 rotate-45 border border-[var(--theme-border)] bg-[var(--theme-surface)]"></div>
                  </div>
                  <h4 className={`${theme.fontSerif} text-xl mb-2`}>{data.events?.akad?.venueName}</h4>
                  <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)] mb-8 leading-relaxed`}>{data.events?.akad?.address}</p>

                  {data.events?.akad?.googleMapsUrl && (
                    <a href={data.events?.akad?.googleMapsUrl} target="_blank" rel="noreferrer" className="inline-block rounded-sm bg-[var(--theme-primary)] text-[var(--theme-bg)] px-6 py-3 text-xs font-bold uppercase tracking-widest hover:brightness-110 transition-colors">
                      View Location
                    </a>
                  )}
                </div>
            ...
            {data.events?.resepsi?.enabled !== false && (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)] mb-8 leading-relaxed`}>{data.events?.resepsi?.address}</p>

                  {data.events?.resepsi?.googleMapsUrl && (
                    <a href={data.events?.resepsi?.googleMapsUrl} target="_blank" rel="noreferrer" className="inline-block rounded-sm bg-[var(--theme-primary)] text-[var(--theme-bg)] px-6 py-3 text-xs font-bold uppercase tracking-widest hover:brightness-110 transition-colors">
                      View Location
                    </a>
                  )}
                </div>
        ...
        {data?.gallery && data.gallery.length > 0 && (
          <section id="gallery-section" className="py-24 px-4 bg-[var(--theme-bg)] relative overflow-hidden">
            ...
            >
              <h2 className={`text-center ${theme.fontSerif} text-3xl md:text-5xl mb-16 tracking-widest`}>
                <AccentText>Galeri Kenangan</AccentText>
              </h2>
        ...
        {data?.gifts && data.gifts.length > 0 && (
          <section className="py-20 px-6">
            ...
            >
              ...
              <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)] mb-10 leading-relaxed`}>
              ...
              <div className="space-y-6">
                {data.gifts.map((gift, idx) => (
                  <div key={idx} className="bg-[var(--theme-surface)] p-6 rounded-lg border border-[var(--theme-border)] shadow-lg relative group overflow-hidden">

                    <div className="flex justify-between items-start mb-8">
                       <div className={`${theme.fontSerif} tracking-widest text-sm uppercase`} style={{ color: 'var(--theme-primary)' }}>{gift.name}</div>
                       <div className="text-[10px] text-[var(--theme-muted)] font-mono">{gift.type === 'address' ? 'ADDRESS' : 'PLATINUM'}</div>
                    </div>
                    <div className="text-left">
                       <p className="font-mono text-xl tracking-widest text-[var(--theme-text)] mb-2">{gift.accountNumber}</p>
                       <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)] uppercase`}>{gift.accountHolder}</p>
                    </div>
                    {gift.type !== 'address' && (
                       <button
        ...
        {/* RSVP SECTION */}
        <section className="py-20 px-6 bg-[var(--theme-bg)] border-t border-[var(--theme-border)]">
          <motion.div 
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          >
            ...
            <form onSubmit={handleRSVPSubmit} className="bg-[var(--theme-surface)] p-6 rounded-lg border border-[var(--theme-border)] relative overflow-hidden group">
              <div className="absolute top-0 left-0 w-full h-[2px] bg-[var(--theme-primary)] opacity-50"></div>
              ...
              {rsvpSuccess ? (
                <div className="text-center py-8 relative z-10">
                  ...
                  <p className={`${theme.fontSerif} text-xl text-[var(--theme-text)]`}>Terima kasih atas konfirmasi Anda.</p>
                </div>
              ) : (
                <div className="space-y-5 relative z-10">
                  <div>
                    <label className={`${theme.fontSans} block text-xs uppercase tracking-widest text-[var(--theme-muted)] mb-2`}>Nama Lengkap</label>
                    <input
                      type="text"
                      required
                      value={rsvpGuestName}
                      ...
                      disabled={!!guest}
                      className="w-full bg-[var(--theme-surface)] border border-[var(--theme-border)] rounded px-4 py-3 text-sm text-[var(--theme-text)] focus:outline-none focus:border-[var(--theme-primary)] transition-colors"
                      placeholder="Masukkan nama Anda"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className={`${theme.fontSans} block text-xs uppercase tracking-widest text-[var(--theme-muted)] mb-2`}>Kehadiran</label>
                      <select
                        value={rsvpStatus}
                        onChange={(e) => setRsvpStatus(e.target.value as any)}
                        className="w-full bg-[var(--theme-surface)] border border-[var(--theme-border)] rounded px-4 py-3 text-sm text-[var(--theme-text)] focus:outline-none focus:border-[var(--theme-primary)] transition-colors appearance-none"
                      >
                    ...
                    <div>
                      <label className={`${theme.fontSans} block text-xs uppercase tracking-widest text-[var(--theme-muted)] mb-2`}>Jumlah Tamu</label>
                      <input
                        type="number"
                        min="1"
                  ...
                  <div>
                    <label className={`${theme.fontSans} block text-xs uppercase tracking-widest text-[var(--theme-muted)] mb-2`}>Ucapan & Doa</label>
                    <textarea
                      required
                      value={rsvpWishes}
                      onChange={(e) => setRsvpWishes(e.target.value)}
                      rows={3}
                      className="w-full bg-[var(--theme-surface)] border border-[var(--theme-border)] rounded px-4 py-3 text-sm text-[var(--theme-text)] focus:outline-none focus:border-[var(--theme-primary)] transition-colors resize-none"
            ...
            {rsvps && rsvps.length > 0 && (
              <div className="mt-12 space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {rsvps.map((rsvp, idx) => (
                  <div key={idx} className="bg-[var(--theme-surface)] border border-[var(--theme-border)] p-4 rounded-xl">
                    <div className="flex justify-between items-start mb-2">
                      <span className={`${theme.fontSerif} text-[var(--theme-text)] font-bold`}>{rsvp.guestName}</span>
                    ...
                    <p className={`${theme.fontSans} text-xs text-[var(--theme-muted)]`}>"{rsvp.wishes}"</p>
                    <span className="text-[9px] text-[var(--theme-muted)] mt-3 block">{new Date(rsvp.timestamp).toLocaleDateString('id-ID')}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {/* FOOTER */}
        <footer className="bg-[var(--theme-surface)] py-16 text-center border-t border-[var(--theme-border)]">
          <h2 className={`${theme.fontSerif} text-4xl mb-4 text-[var(--theme-secondary)]`}>{data?.couple?.groom?.nickname} & {data?.couple?.bride?.nickname}</h2>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="bg-[var(--theme-surface)] border border-white/10 rounded-full px-6 py-3 shadow-[0_10px_40px_rgba(0,0,0,0.8)] flex items-center gap-6">
              <button onClick={(event) => event.currentTarget.ownerDocument.defaultView?.scrollTo({ top: 0, behavior: 'smooth' })} className="flex flex-col items-center gap-1 text-[var(--theme-muted)] hover:text-[var(--theme-primary)] hover:-translate-y-1 transition-all" aria-label="Kembali ke awal">
                <Heart size={22} />
              </button>
              <button onClick={(event) => event.currentTarget.ownerDocument.getElementById('events-section')?.scrollIntoView({ behavior: 'smooth' })} className="flex flex-col items-center gap-1 text-[var(--theme-muted)] hover:text-[var(--theme-primary)] hover:-translate-y-1 transition-all" aria-label="Lihat acara">
                <Calendar size={22} />
              </button>
              <button onClick={(event) => event.currentTarget.ownerDocument.getElementById('gallery-section')?.scrollIntoView({ behavior: 'smooth' })} className="flex flex-col items-center gap-1 text-[var(--theme-muted)] hover:text-[var(--theme-primary)] hover:-translate-y-1 transition-all" aria-label="Lihat galeri">
                <MapPin size={22} />
              </button>
