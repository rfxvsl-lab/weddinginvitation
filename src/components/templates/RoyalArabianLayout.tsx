                ...
                {data?.gifts && data.gifts.length > 0 ? (
                    data.gifts.map((gift, i) => (
                        <div key={i} className="mb-4">
                            ...
                            <p className="font-modern text-xs uppercase text-gray-400">{gift.name} â€¢ {gift.accountHolder}</p>
                            {gift.type !== 'address' && (
                                <button
...
const RoyalArabianLayout: React.FC<RoyalArabianProps> = ({ data, guest, onAddRSVP, rsvps, embedded = false }) => {
    const [stage, setStage] = useState<'envelope' | 'hero' | 'content'>(embedded ? 'content' : 'envelope'); // envelope -> hero -> content
