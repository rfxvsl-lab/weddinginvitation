  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 border border-indigo-150 text-indigo-700 bg-indigo-50/40 hover:bg-indigo-50 rounded-xl text-xs font-semibold scale-100 hover:scale-102 active:scale-98 transition-all cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold scale-100 hover:scale-102 active:scale-98 transition-all cursor-pointer"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="p-4 bg-white rounded-2xl border border-slate-100 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          <div className="p-3 rounded-xl bg-blue-50 text-blue-600">
            <Eye className="w-5 h-5" />
          </div>
          <div>
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Dibuka</span>
            <span className="text-lg font-bold text-slate-800 font-mono">{analytics.viewsCount}x</span>
          </div>
        ...
        <div className="p-4 bg-white rounded-2xl border border-slate-100 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          ...
          <div>
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Konfirmasi Hadir</span>
            <span className="text-lg font-bold text-slate-800 font-mono">{countHadir} Tamu</span>
          </div>
          <div className="absolute right-0 bottom-0 translate-x-2 translate-y-2 opacity-5 pointer-events-none group-hover:scale-110 transition-transform">
        ...
        <div className="p-4 bg-white rounded-2xl border border-slate-100 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          ...
          <div>
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Estimasi Porsi Pax</span>
            <span className="text-lg font-bold text-slate-800 font-mono">{totalPaxAttending} Orang</span>
          </div>
          <div className="absolute right-0 bottom-0 translate-x-2 translate-y-2 opacity-5 pointer-events-none group-hover:scale-110 transition-transform">
        ...
        <div className="p-4 bg-white rounded-2xl border border-slate-100 flex items-center gap-4 shadow-sm relative overflow-hidden group">
          ...
          <div>
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Absen / Ragu-ragu</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="p-5 bg-white border border-slate-100 rounded-3xl shadow-sm lg:col-span-2 space-y-4">
          ...
          {/* X Axis Labels */}
          <div className="flex justify-between px-1 text-[9px] font-mono font-semibold text-slate-400">
        ...
        <div className="p-5 bg-white border border-slate-100 rounded-3xl shadow-sm space-y-4">
          <h4 className="font-semibold text-slate-800 text-xs uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-100 pb-2.5">
          ...
          <div className="space-y-3 max-h-[145px] overflow-y-auto pr-1">
            {analytics.visitorLogs.map((log) => {
              ...
              return (
                <div key={log.id} className="flex items-start justify-between text-[11px] hover:bg-slate-50 p-1.5 rounded-lg transition">
                  ...
                  <span className="text-[9px] font-mono text-slate-400 mt-0.5">{log.timestamp}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left font-sans border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {rsvps.length > 0 ? (
                rsvps.map((entry) => (
                  <tr key={entry.id} className="text-xs hover:bg-slate-50 transition">
                    ...
                    <td className="px-4 py-3 text-slate-500 max-w-sm whitespace-pre-wrap leading-relaxed truncate hover:text-clip hover:whitespace-normal">
                      "{entry.wishes}"
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className="p-1 px-2 rounded-lg border border-slate-150 hover:border-rose-250 text-slate-400 hover:text-rose-500 bg-white hover:bg-rose-50/50 transition duration-300"
                        title="Hapus RSVP"
                      >
